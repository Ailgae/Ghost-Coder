...
const serverUrlEl = $('serverUrl'), modelEl = $('model'), statusEl = $('status'), messagesEl = $('messages');
const composerForm = $('composerForm'), composerInput = $('composerInput'), sendBtn = $('sendBtn'), stopBtn = $('stopBtn');
const connectionBtn = $('connectionBtn'), serverStatusTextEl = $('serverStatusText'), projectsListEl = $('projectsList'), chatTitleEl = $('chatTitle');
const projectModal = $('projectModal'), projectForm = $('projectForm'), projectNameEl = $('projectName'), projectPathEl = $('projectPath');
let sending = false, projects = [], activeProjectId = null, activeChatId = null, editingProjectId = null;
...
function addBubble(role, text) { if (!text?.trim()) return; const element = document.createElement('div'); element.className = `bubble ${role}`; element.textContent = text; messagesEl.append(element); scrollToBottom(); }
...
connectionBtn.onclick = () => { $('connectionModal').hidden = false; serverUrlEl.focus(); }; $('cancelConnectionBtn').onclick = () => { $('connectionModal').hidden = true; };
...
composerForm.onsubmit = async event => { event.preventDefault(); const text = composerInput.value.trim(); if (sending || !text || !modelEl.value) return; if (!activeProjectId || !activeChatId) return setStatus('Add a project before sending a message.'); composerInput.value = ''; composerInput.style.height = 'auto'; addBubble('user', text); setSending(true); setStatus('Working…'); const result = await window.vibe.sendMessage({ projectId: activeProjectId, chatId: activeChatId, text }); if (result.ok) { addBubble('agent', result.content); chatTitleEl.textContent = result.title; } else addBubble('error', `Error: ${result.error}`); setStatus(result.ok ? 'Ready' : 'Error'); setSending(false); stopBtn.disabled = false; await renderProjects(); };
window.vibe.onEvent(event => { if (event.chatId !== activeChatId) return; if (event.type === 'note') addBubble('agent', event.content); else if (event.type === 'thinking') addAction('Thinking', event.content); else if (event.type === 'tool_call') addAction(describeToolCall(event.name, event.args), JSON.stringify(event.args, null, 2)); else if (event.type === 'tool_result') addAction(`${event.result?.ok ? '✓' : '✗'} ${event.name} result`, JSON.stringify(event.result, null, 2)); });
